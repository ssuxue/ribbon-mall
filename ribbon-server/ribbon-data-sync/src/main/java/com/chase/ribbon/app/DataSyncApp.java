...
import com.chase.ribbon.service.EventHandleContext;
...
public abstract class DataSyncApp implements ActionByEventType{
    ...
    private final Logger logger = LoggerFactory.getLogger(DataSyncApp.class);
    ...
    public void work(){
        while (RUNNING){
            ...
            if (!(batchId == -1 || size == 0)) {
                logger.info("处理 batchId 为 {} 的数据",batchId);
                ...
                if (handlers != null) {
                    for (EventHandleContext handler : handlers) {
                        ...
                        try {
                            ...
                            logger.error("处理 batchId 为 {} 的数据发生异常 ",batchId);
                            e.printStackTrace();
    ...
    private List<EventInfo> createEventInfos(List<CanalEntry.Entry> entrys) {
        ...
        for (CanalEntry.Entry entry : entrys) {
            ...
            logger.info("===========binlog[{} : {}] ,tableName[{} : {}] , eventType : {}",
